...
var TextCleaner =
{
  ...
  sanitizeWikiText : function (input, only_thumbs)
  {
    ...
    function sanitize (s, with_links, caption_level, allow_thumbs, break_at_pipe, with_tables)
    {
      if (!s || s.length == 0) {
        if (caption_level > 0) {
      ...
      while (s.length > 0 && !get_out) {
        ...
        switch (ch) {
          ...
          case '\x05':
            ...
            // Fall through
          case '\x01':
            // Start of template or table
            result = result + s.substring (0, next+1);
            push_end (ch == '\x01' ? '\x02' : '\x06');
          ...
          case '[':
            {
              ...
              if (i+5 < s.length) {
                initial = s.substr (i, 6).toLowerCase ();
                if (initial != 'image:') {                  
                  initial = initial.substr (0, 5);
                  if (initial != 'file:') initial = null;
                }
              }
              ...
              if (k < 0) {
                ...
                if (initial == null) {
                  ...
                  l = q.search(/[\x01\x02\x03[\x04\]\{\}\x05\x06]/);
                  if (l < 0) l = q.length;
                  if (l+1 < q.length) q = q.substring (0, l+1);
                  var is_thumb = q.search (/\|\s*thumb(nail)?\s*[\|\x04]/) >= 0;
                  var is_small = q.search (/\|\s*[12]?\d?\dpx\s*[\|\x04]/) >= 0; // Maximum 299px
                  ...
                  is_thumb = allow_thumbs && (is_thumb || is_small);
    ...
    s = sanitize (s, true, 0, true, false, true);
    // with links, allow thumbs, don't break at pipe, allow tables

...
namespace Com.Hertkorn.Framework.SourceCodeManagement.SourceTree
{
    public class SourceTree : ISourceTreeRootDirectory
    {
        ...
        private class Directory : ISourceTreeDirectory
        {
            ...
            /// Flat tree
            /// </summary>
            /// <returns>child tree flatten - not containing this.</returns>
            private IList<SourceTree.Directory> FlattenChildTree()
            {
                List<SourceTree.Directory> flattenTree = new List<SourceTree.Directory>();
                flattenTree.AddRange(m_childDirectoryListe);
                foreach (var item in m_childDirectoryListe)
                {
                    flattenTree.AddRange(item.FlattenChildTree());
                }
                return flattenTree;
            }
